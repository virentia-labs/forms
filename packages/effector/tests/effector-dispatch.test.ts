...
describe("formToEffector — dispatch direction (effector → virentia)", () => {
  describe("model.fill", () => {
  ...
  describe("model.clearOuterErrors / clearInnerErrors", () => {
  ...
  describe("model.forceUpdateSnapshot", () => {
  ...
  describe("array field item target()", () => {
    const makeTags = (initial: readonly string[] = ["a", "b", "c"]) =>
      setupForm(createForm({ schema: { tags: createArrayField<string>(initial) } }));
  ...
  describe("nested shape child leaf target()", () => {
    const makeProfile = () =>
      setupForm(
        createForm({
          schema: {
  ...
  describe("nested array-of-shapes item leaf target()", () => {
    const makeUsers = () =>
      setupForm(
        createForm({
          schema: {
  ...
  describe("props() threading", () => {
    ...
    it("the `where` predicate's props argument is always `undefined` at runtime", async () => {
  ...
  describe("change-strategy forms", () => {
