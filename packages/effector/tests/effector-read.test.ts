 ...
 * module, exercised in `effector-dispatch.test.ts`.
 *
...
describe("formToEffector — read direction (virentia → effector)", () => {
  describe("initial seeding", () => {
    ...
    it("an empty schema seeds empty stores and an empty fields lens", () => {
  ...
  describe("store mirroring", () => {
    ...
    it("mirrors $outerErrors and $errors from fill({ errors }) and flips $isValid false", async () => {
      const h = setupForm(createForm({ schema: { email: createField("") } }));
    ...
    it("clears the outer error and restores $isValid on clearOuterErrors", async () => {
      const h = setupForm(createForm({ schema: { email: createField("") } }));
      await h.drive(() => h.form.fill({ errors: { email: "bad" } }));
  ...
  describe("leaf field lens units", () => {
  ...
  describe("nested plain group lens", () => {
  ...
  describe("array collection lens", () => {
    const makeTags = () =>
    ...
    it("`where` over object-valued items spreads the item value onto the predicate data", () => {
      const h = setupForm(
        createForm({
          schema: {
            people: createArrayField([{ name: "A" }, { name: "B" }], {
  ...
  describe("shape collection lens", () => {
    const makeProfile = () =>
      setupForm(
        createForm({
          schema: {
    ...
    it("`where` matches on a child's leaf `value`", () => {
      const h = makeProfile();
      const matched = h.drive(() =>
