...
describe("defineField", () => {
  describe("copying", () => {
    it("returns a distinct object that shares non-function property references", () => {
      const blob = { nested: 1 };
      const original = {
        kind: "field",
    ...
    it("wraps the copy's method without upgrading the original's", () => {
      const blob = { nested: 1 };
      const original = {
        kind: "field",
        async fill(_next: unknown) {},
    ...
    it("returns a distinct empty copy for an empty object", () => {
      const src = {};
  ...
  describe("method wrapping", () => {
    it("wraps a plain method into an effect named `${kind}.${key}`", () => {
    ...
    it("uses an empty-string kind verbatim rather than defaulting to `field`", () => {
  ...
  describe("skipped members", () => {
    ...
    it("leaves stores, events and existing effects untouched", () => {
      const state = store(1);
  ...
  describe("invoking a wrapped method", () => {
    it("returns a promise that runs the underlying method when awaited", async () => {
    ...
    it("forwards the payload to the underlying method", async () => {
      const appScope = scope();
      let seen: unknown;
      const defined = define({
        kind: "k",
      ...
      await scoped(appScope, async () => {
        await defined.plain(21);
      });

      expect(seen).toBe(21);
    });

    it("resolves with the underlying method's return value", async () => {
      const appScope = scope();
      const defined = define({
        kind: "k",
        // sync function is still a function → still wrapped
    ...
    it("keeps the ambient scope alive across the method's await tail", async () => {
      const appScope = scope();
      const defined = define({
        kind: "sc",
        async work() {
    ...
    it("rejects the awaited effect when the wrapped method throws", async () => {
      const appScope = scope();
      const defined = define({
        kind: "err",
        async boom() {
    ...
    it("drops every argument after the first", async () => {
      const appScope = scope();
  ...
  describe("unusual keys", () => {
    it("copies but does not wrap symbol-keyed function props", () => {
    ...
    it("drops inherited prototype methods, keeping only own enumerable keys", () => {
  ...
  describe("idempotency", () => {
    it("reuses an already-wrapped method instead of re-wrapping it", () => {
    ...
    it("stays stable when applied three times", () => {
  ...
  describe("on a createField instance", () => {
    it("leaves the field's existing units untouched", () => {
    ...
    it("leaves the field's sync accessors untouched", () => {
      const raw = createField("x");
      const defined = define(raw);
    ...
    it("wraps the plain clear helpers into effects named by field kind", () => {
      const raw = createField("x");
      const defined = define(raw);
    ...
    it("keeps a wrapped clear effect isolated to the scope it runs in", async () => {
    ...
    it("surfaces its validation error through the enclosing form", async () => {
      const appScope = scope();
...
describe("fieldType", () => {
  it("wraps the created field's methods into effects named by kind", () => {
    const ft = fieldType({
  ...
  it("forwards its call argument to the factory", async () => {
    const appScope = scope();
    const ft = fieldType({
      create: (initial: number) =>
        ({
  ...
  it("returns a fresh wrapped instance per call", () => {
    const ft = fieldType({
  ...
  it("names effects by the field's own kind, ignoring the config kind", () => {
    const ft = fieldType({
  ...
  describe("extend", () => {
    it("wraps an inherited method once, keeping the base kind", () => {
    ...
    it("wraps a newly added method with the extension's kind", () => {
      const base = fieldType({
        create: (n: number) =>
          ({
            kind: "b",
    ...
    it("names effects by the field's own kind, ignoring the extension config kind", () => {
      const base = fieldType({
    ...
    it("flows the label argument into the effect name", () => {
    ...
    it("passes the initial argument through to the base factory", async () => {
      const appScope = scope();
      const numberType = fieldType({ create: (initial: number) => createField(initial) });
      const relabelled = numberType.extend({
        create(base, label: string, initial: number) {
    ...
    it("names each stacked layer's method by that layer's kind", () => {
    ...
    it("transforms the value through each stacked layer's awaitable effect", async () => {
      const appScope = scope();
      const primitive = fieldType({ create: createField });
      const trimmed = primitive.extend({
        create(base, initial: string) {
    ...
    it("names every method across a triple extend chain by its layer's kind", () => {
    ...
    it("preserves the caller's scope after running every method in a triple chain", async () => {
      const appScope = scope();
      const l1 = fieldType({ create: createField });
      const l2 = l1.extend({
        create(base, initial: string) {
