...
// Idiom: build instances OUTSIDE a scope, mutate INSIDE `scoped(appScope, ...)`.
describe("createArrayField", () => {
  describe("construction", () => {
    ...
    it("seeds the values, keys item fields by index, and serializes null errors", async () => {
      const appScope = scope();
    ...
    it("aliases fields to itemFields, exposing the same instances", async () => {
      const appScope = scope();
    ...
    it("default createItem folds only the value, ignoring the index", async () => {
      const appScope = scope();
  ...
  describe("structural mutations", () => {
    it("appends, prepends and inserts raw values, emitting changed and errorsChanged per op", async () => {
      const appScope = scope();
    ...
    it("appends, prepends and inserts a field instance, keeping its identity", async () => {
      const appScope = scope();
      const tags = createArrayField(["a"]);
    ...
    it("pushing a field instance into an empty collection emits changed and errorsChanged", async () => {
      const appScope = scope();
      const tags = createArrayField<string>([]);
      const changed = watchCalls(tags.changed);
      const errorsChanged = watchCalls(tags.errorsChanged);
    ...
    it("insert clamps the index into [0, length]", async () => {
      const appScope = scope();
      const tags = createArrayField(["b"]);
    ...
    it("remove out of range leaves the collection and events untouched", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
      const changed = watchCalls(tags.changed);
      const errorsChanged = watchCalls(tags.errorsChanged);
    ...
    it("remove of a middle index keeps surviving instance identities", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b", "c"]);
      const changed = watchCalls(tags.changed);
    ...
    it("pop removes the last item and no-ops on an empty collection", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
      const changed = watchCalls(tags.changed);
    ...
    it("replace in range fills the existing instance", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
    ...
    it("replace in range with a field instance swaps the instance", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
    ...
    it("replace out of range appends for a high index and prepends for a negative index", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
    ...
    it("move keeps instance identity, clamps the target, and no-ops on an out-of-range source", async () => {
      const appScope = scope();
    ...
    it("swap keeps instance identity and no-ops when either index is out of range", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b", "c", "d"]);
      const changed = watchCalls(tags.changed);
    ...
    it("swap of an index with itself re-emits changed, leaving order and identity intact", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b", "c"]);
      const changed = watchCalls(tags.changed);
    ...
    it("clear empties the collection and resets the own error boxes", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
  ...
  describe("fill", () => {
    it("reuses instances, extends beyond them, drops trailing items, and emits changed once", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
      const changed = watchCalls(tags.changed);
    ...
    it("the change strategy runs validation as part of fill", async () => {
      const appScope = scope();
      const tags = createArrayField(["a"], {
  ...
  describe("reset", () => {
    it("recreates item instances and restores the initial values", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
      ...
      await scoped(appScope, async () => {
        ...
        // brand-new instances: none of the pre-reset instances survive
  ...
  describe("error model", () => {
    it("distributes an inner array to items and mirrors it through errors precedence", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
    ...
    it("a scalar inner error is stored on the own box and shadows per-item errors", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
    ...
    it("outer scalar shadows inner scalar, which shadows per-item errors", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
    ...
    it("an inner-error array shorter than the items leaves the trailing items untouched", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b", "c"]);
    ...
    it("setInnerErrors longer than the items ignores the extra entries", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
    ...
    it("distributes nested shape item object errors", async () => {
      const appScope = scope();
    ...
    it("serialize reflects the live outer error and the current values", async () => {
      const appScope = scope();
      const tags = createArrayField(["a", "b"]);
  ...
  describe("validation", () => {
    it("validates every item in order, then the array validators on read()", async () => {
      const appScope = scope();
      const calls: string[] = [];
    ...
    it("an array result from the array validator distributes to items, leaving the own inner box null", async () => {
      const appScope = scope();
      const tags = createArrayField(["", "ok"], {
        createItem(value: string) {
          return createField(value, {
    ...
    it("a non-array result is stored on the own inner box and shadows item errors", async () => {
      const appScope = scope();
      const tags = createArrayField(["", "ok"], {
        createItem(value: string) {
          return createField(value, {
    ...
    it("emits validated with the read value when the collection is valid", async () => {
      const appScope = scope();
    ...
    it("emits validationFailed with the read value when the collection is invalid", async () => {
      const appScope = scope();
    ...
    it("tracks the pending flag across an async run and reflects the failed result in isValid", async () => {
      const appScope = scope();
    ...
    it("the pending flag is true while an item validator is pending", async () => {
      const appScope = scope();
      const pending = deferred<string | null>();
      const tags = createArrayField(["x"], {
        createItem: (value: string) =>
    ...
    it("revalidates on an array-level ctx.read dependency change, preserving item errors without leaking scope", async () => {
      const appScope = scope();
