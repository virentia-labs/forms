...
describe("createField (exhaustive)", () => {
  // ---------------------------------------------------------------------------
  // A.1 Construction
  // ---------------------------------------------------------------------------
  ...
  describe("error model", () => {
    ...
    it("BUG: reading the combined `error` computed poisons the next synchronous errorsChanged emit", async () => {
      const appScope = scope();
      const field = createField("");
      const errorsChanged = watchCalls(field.errorsChanged);
    ...
    it("F3: changeError writes the OUTER channel (not inner)", async () => {
      const appScope = scope();
      const field = createField("");
    ...
    it("F2: isValid is STRICTLY error === null (empty-string error is invalid)", async () => {
      const appScope = scope();
      const field = createField("");
    ...
    it("F1: setInnerErrors(undefined) makes error === undefined and isValid false", async () => {
      const appScope = scope();
      const field = createField("");
  ...
  // ---------------------------------------------------------------------------
  describe("reset", () => {
    it("F6: reset restores value/inner/meta/focus and outer -> options.error (NOT null)", async () => {
      const appScope = scope();
  ...
  describe("validation", () => {
    ...
    it("detects effect validators and calls them as ({value,ctx},{signal}) — handler signal is an AbortSignal distinct from ctx.signal", async () => {
      const appScope = scope();
      ...
      await scoped(appScope, async () => {
        ...
        // NOTE (BUG vs intent): runValidationUnit calls the effect as
        // `validator({value,ctx}, { signal: ctx.signal })`, i.e. it INTENDS the
        // handler to receive ctx.signal. In practice the effect runtime hands the
        // handler its OWN execution signal, so the two are distinct objects.
  ...
  // ---------------------------------------------------------------------------
  describe("strategies", () => {
    it("F12: change strategy validates INLINE (error is current right after await fill)", async () => {
      const appScope = scope();
      const field = createField("", {
        validate: (v: string) => (v ? null : "Required"),
    ...
    it("F12: focus/blur strategies validate via reaction; awaiting the event settles it", async () => {
      const appScope = scope();
