...
describe("@virentia/forms-react (exhaustive)", () => {
  // =========================================================================
  // useField
  // =========================================================================
  describe("useField", () => {
    it("exposes value, errors, innerErrors, outerErrors, isValid, isValidationPending and view", () => {
    ...
    it("re-renders after a scoped fill committed outside React", async () => {
    ...
    it("re-renders after the field `change` event fires in scope", async () => {
      const field = createField("a");
      const { appScope } = renderWithScope(<Value field={field} />);
    ...
    it("re-renders errors after validate through the scoped method", async () => {
    ...
    it("re-renders across setInnerErrors / setOuterErrors / clearOuterErrors / clearInnerErrors", async () => {
    ...
    it("re-renders after reset restores value and clears errors", async () => {
    ...
    it("runs the returned methods inside the provider scope (useScopedMethod)", async () => {
      const field = createField("a");
    ...
    it("passes through the `view` of a custom field contract", () => {
      const custom = {
        kind: "custom",
        state: store("val"),
        view: { hint: "H" },
    ...
    it("exposes the normalized field object (FieldView.field passthrough)", () => {
      const field = createField("hello");
    ...
    it("renders a leaf field (scalar value, string|null errors)", async () => {
    ...
    it("renders a shape field (object value / object errors)", async () => {
    ...
    it("renders an array field (list value / null errors) and fills through view.fill", async () => {
  ...
  describe("useForm", () => {
    ...
    it("exposes values, errors, innerErrors, outerErrors, snapshot, isChanged, isValid, isValidationPending", () => {
      const form = makeForm();
    ...
    it("fill updates values, validates (change strategy), and flips isChanged/isValid", async () => {
      const form = makeForm();
    ...
    it("reset restores values, clears errors and isChanged", async () => {
      const form = makeForm();
    ...
    it("validate() surfaces errors without a change (manual strategy)", async () => {
    ...
    it("submit() runs validation before submitting", async () => {
      const form = createForm({
        schema: {
          name: createField("abcdef", {
            validate: (value: string) => (value.length > 3 ? "Too long" : null),
    ...
    it("clearOuterErrors clears externally supplied outer errors", async () => {
    ...
    it("clearInnerErrors clears validation (inner) errors", async () => {
      const form = createForm({
        schema: {
          name: createField("abcdef", {
            validate: (value: string) => (value.length > 3 ? "Too long" : null),
    ...
    it("forceUpdateSnapshot rebases the snapshot and clears isChanged", async () => {
      const form = createForm({ schema: { name: createField("") } });
    ...
    it("passes the form model through by identity (FormView.form)", () => {
      const form = makeForm();
      let captured: unknown;
    ...
    it("form methods run inside the provider scope", async () => {
      const form = createForm({ schema: { name: createField("") } });
      const otherScope = scope();
  ...
  // =========================================================================
  // useWizard / useWizardForm
  // =========================================================================
  describe("useWizard / useWizardForm", () => {
    function makeWizard() {
      const form = createForm({
        schema: {
          email: createField("", {
    ...
    it("blocks next on an invalid step and advances once valid", async () => {
    ...
    it("goTo jumps forward when intermediate steps validate", async () => {
      const { form, wizard } = makeWizard();
      const { appScope } = renderWithScope(<WizardView wizard={wizard} />);
    ...
    it("complete marks every step completed once all are valid", async () => {
      const { form, wizard } = makeWizard();
      const { appScope } = renderWithScope(<WizardView wizard={wizard} />);
    ...
    it("reset returns to the first step and clears completed", async () => {
      const { form, wizard } = makeWizard();
      const { appScope } = renderWithScope(<WizardView wizard={wizard} />);
    ...
    it("useWizardForm is the same hook as useWizard", () => {
      expect(useWizardForm).toBe(useWizard);
    });

    it("useWizardForm renders wizard state identically", () => {
      const { wizard } = makeWizard();
    ...
    it("passes the wizard model through by identity (WizardView.wizard)", () => {
      const { wizard } = makeWizard();
      let captured: unknown;
  ...
  describe("scope isolation", () => {
    it("holds independent values under two different ScopeProviders", async () => {
    ...
    it("does not re-render the scope-B component when scope A mutates (and vice versa)", async () => {
      const field = createField("init");
      const scopeA = scope();
      const scopeB = scope();
      const aCount = { n: 0 };
  ...
  // =========================================================================
  describe("useSyncExternalStore correctness", () => {
    it("shows no tearing: DOM matches the current scoped snapshot after rapid writes", async () => {
    ...
    it("unsubscribes on unmount (no update-after-unmount / act warnings)", async () => {
    ...
    it("re-subscribes when the field prop identity changes (useCallback deps)", async () => {
    ...
    it("does not infinitely re-render array/shape fields whose snapshots are freshly built", async () => {
